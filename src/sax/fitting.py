...
def _johnson_su_transform(
    x: jnp.ndarray, gamma: float, delta: float, xi: float, lambda_: float
) -> jnp.ndarray:
...
def _fit_johnson_parameters(x: jnp.ndarray, method: str = "su") -> dict[str, float]:
...
def _transform_covariates(
    df: pd.DataFrame, target_col: str, transform_method: str = "johnson_su"
    ...
    for col in continuous_cols:
        if df[col].nunique() > 10:  # Only transform if enough unique values
            ...
            transform_params[col] = {"method": transform_method, "params": params}
...
def _compute_loss(
    ...
    return base_loss + penalty_lambda * penalty
...
def neural_fit(
    ...
    hidden_dims: tuple[int, ...] = (10, 5),
    activation: str = "tanh",
    transform_covariates: bool = True,
    transform_method: str = "johnson_su",
    robust_fit: bool = False,
    penalty_method: str = "squared",
    penalty_lambda: float = 0.01,
    num_tours: int = 5,
    ...
    transform_covariates : bool
        Whether to transform continuous variables to near normality
    transform_method : str
        Transformation method: 'johnson_su' or 'johnson_sb'
    robust_fit : bool
        Whether to use least absolute deviations instead of least squares
    penalty_method : str
        Penalty method: 'squared', 'absolute', 'weight_decay', 'no_penalty'
    penalty_lambda : float
        Penalty parameter λ
    num_tours : int
        Number of restart tours with different random starting points
    learning_rate : float
    ...
    for tour in range(num_tours):
        ...
        try:
            ...
            tour_results.append(
                {"tour": tour, "val_loss": final_val_loss, "history": history}
            )
            ...
            warnings.warn(f"Tour {tour} failed: {e!s}")
            continue

    if best_params is None:
        raise RuntimeError("All tours failed. Try adjusting hyperparameters.")

    # Create prediction function
    def predict(X_new: jnp.ndarray) -> jnp.ndarray:
    ...
    return {
        ...
        "performance": {
            metric_name.lower(): float(final_loss),
...
def predict_neural_model(
    model_result: dict[str, Any], X_new: pd.DataFrame | jnp.ndarray
) -> jnp.ndarray:
    ...
    X_new : pd.DataFrame or jnp.ndarray
